...
\item Class \Rclass{Pipeline} is a subclass of class \Rclass{Command}, and a
  ...
  sequence of \Rclass{Protocols}. A \Robject{Pipeline} object provides
  \textbf{Command} interface which has a \Rfunction{perform} method, it is also
  similar to \textit{invoker} in \textit{Command} pattern, accept different
  \textit{Command}(the \Robject{Protocol}) in queued order. The
  \Rfunction{Pipeline} constructor is more like a \textit{client} in
...
\item A \Rclass{PipelineData} is also a virtual class, just represent a
  ...
  equally applied to any data set have a slot or attribute called 'pipeline'.
...
\item A \Rclass{Protocol}(method) object performs a particular \Rclass{Stage},
  ...
  object is similar to \textit{receiver} in \textit{Command} pattern, which
  implement method and parameters as \textit{ConcreteCommand} method.
\end{itemize}
